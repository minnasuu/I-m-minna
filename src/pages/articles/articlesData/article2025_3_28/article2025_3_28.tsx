...
export const article2025_3_28 = <ArticleLayout>
  <ArticleSectionLayout>
    <ArticlePartLayout>
      <ArticleTitle type={'h1'} title={'前言：Agent UI 对话式布局有哪些特点？'} />
      ...
      <ArticleDesc useBg toggle toggleTitle='聊天软件的对话式布局特点（展开查看详情）'>
      ...
      <ArticleDesc>综合以上的特点，我认为实现流畅的 AI 产品对话式布局的重点是实现好对话式滚动布局、上下文分页加载、消息流控制三个方面。接下来我会详细地从这两个方面入手，记录我在实现这种布局时的探索与思考。</ArticleDesc>
    </ArticlePartLayout>
  </ArticleSectionLayout>
  ...
  <ArticleSectionLayout>
    <ArticlePartLayout>
      ...
      <ArticleDesc>然而在网页开发的语言里，信息一直是从上往下加载的，信息超过显示区域时，也是从上往下滚动浏览的，这似乎和
        AI 的对话布局刚好<span className="bold highlight">相反</span>，这自然是需要解决的关键问题。</ArticleDesc>
      <div className={'flex column gap-4 items-center'}>
        <div className={'px-12 py-24 width-100 bg-gray border-box'} style={{ height: '240px', overflow: 'auto' }}>
          <div className={'flex both-center fs-12 color-gray-4'}
            style={{ height: '400px', backgroundColor: 'var(--color-border-1)' }}>对话内容
          </div>
        </div>
        <div className={'fs-12 color-gray-4'}>常规的网页滚动布局</div>
      </div>
      <ArticleDesc><span className={'bold'}>接下来一起👀看看别的产品怎么实现的？</span>在记录我的实现方式之前，先一起看看其他
        AI 产品，甚至是聊天产品是怎么实现的？</ArticleDesc>
    </ArticlePartLayout>
    <ArticlePartLayout>
      <ArticleTitle type={'h2'} title={'方案一：常规布局 ➕ 实时滚动到底部'} />
      <ArticleDesc noIndent>案例：<a href={'https://chatgpt.com/'}>ChatGPT</a>、<a href={'https://deepseek.com/'}>DeepSeek</a>、<a href={'https://yuanbao.tencent.com/'}>腾讯元宝</a></ArticleDesc>
      ...
      <ArticleDesc noIndent>核心在于理清<span className="bold">生成时自动滚动</span>与<span className="bold">用户自由滚动</span>的关系与优先级：</ArticleDesc>
      <ArticleDesc>
        <ol>
      ...
      <ArticleDesc>
        <ul>
          ...
          <li>通过判断滚动事件的来源（用户或程序），若用户干预，则将自动滚动的标记。
          </li>
        </ul>
      </ArticleDesc>
      <AgentScroller3 />
      <ArticleDesc>至此，第一种对话式布局的方案就全部实现了，这种方案具有很明显的优点，也是市面上 Agent 产品的主流方案，缺点则需要根据业务需求进行判断。</ArticleDesc>
      <ArticleTitle type={'h3'} title={'优点'} />
      <ArticleDesc>这种实现方式因为<span className="bold">没有改动网页常规布局</span>，而是通过 js 控制滚动容器的位置，所以不会影响到网页滚动布局中的一些元素和交互，如滚动条的位置、展开收起交互等（对比后续方案）。</ArticleDesc>
      <ArticleTitle type={'h3'} title={'缺点'} />
      <ArticleDesc>通过 JS 实现，相对复杂（后面两种方案通过 css 样式即可实现）。</ArticleDesc>
    </ArticlePartLayout>

    <ArticlePartLayout>
      <ArticleTitle type={'h2'} title={'方案二：transform 旋转布局'} />
      <ArticleDesc noIndent>案例：<a href={'https://jimeng.jianying.com/'}>即梦</a>、<a href={'https://yuanbao.tencent.com/'}>腾讯元宝小程序</a></ArticleDesc>
      <ArticleDesc>既然对话式布局与常规布局相反，自然而然就想到了旋转，将常规列表旋转 180度，将常规布局的顶部作为旋转后的底部，即可做到始终定位到底部。</ArticleDesc>
      <ArticleTitle type="h3" title="实现步骤" />
      <ArticleDesc>这种方案实现起来很简单，仅需要设置样式旋转滚动容器和其中内容即可，不需要额外的 js 计算</ArticleDesc>
      <ArticleTitle type="h4" title="step1. 旋转滚动容器和容器内的所有节点：" />
      <div className="flex gap-24 fs-12 color-gray-4">
        <div className="flex-1 bg-gray overflow-auto" style={{ height: '220px' }}>
          <div className="width-50 mx-auto" style={{ height: '320px', background: 'rgba(0,0,0,0.1)' }}><p>测试文字...测试文字..测试文字...测试文字..</p></div>
        </div>
        <div className="flex-1 bg-gray overflow-auto rotate-180" style={{ height: '220px' }}>
          <div className="width-50 mx-auto" style={{ height: '320px', background: 'rgba(0,0,0,0.1)' }}><p>测试文字...测试文字..测试文字...测试文字..</p></div>
        </div>
        <div className="flex-1 bg-gray overflow-auto rotate-180" style={{ height: '220px' }}>
          <div className="width-50 mx-auto" style={{ height: '320px', background: 'rgba(0,0,0,0.1)' }}><p className="rotate-180">测试文字...测试文字..测试文字...测试文字..</p></div>
        </div>
      </div>
      <div className="flex gap-24 fs-12 color-gray-3">
      ...
      <AgentScroller4 />
      <ArticleDesc>可以看到，将滚动容器翻转后，顶部成为了底部，内容从上往下展示变成了从下往上，大致是我们想要的效果了。但是明显还存在一些问题：(第1条可解决)</ArticleDesc>
      <ArticleDesc>
        <ol>
          <li>对话已开始就是从底部开始显示——而我们期望的 Agent对话式布局是对话初始时依然是从顶部开始，只是对话到达底部时向上展开滚动。</li>
          <li>滚动方向与常规操作反向、滚动条位于容器左侧——翻转后滚动条也被旋转到了左侧，容器的滚动方向也与常规相反，鼠标向上滚动时，内容才会向上滚动。</li>
        </ol>
      </ArticleDesc>
      <ArticleTitle type="h4" title="step2. 问题 1 解决——营造视觉上对话从顶部开始，到达底部后向上展开滚动。" />
      <ArticleDesc>第1个问题解决起来也相对简单，只需要设置滚动容器的高度即可，在内容未到达底部时，高度始终与内容相同，内容到达底部时通过最大高度使容器开始滚动。</ArticleDesc>
      <ArticleCode language="css" codeStr={codeString}></ArticleCode>
      <AgentScroller4 fix />
      <ArticleTitle type="h4" title="step3. 问题 2 解决——修复滚动条同时被旋转到左侧的问题。" />
      <ArticleDesc>实现这种方案用到了 css 的变换属性 transform 的 rotate 函数，transform 还有一个函数 scale —— 用于缩放元素，将 scale 的值设置为 "-1" 时可实现镜像翻转，所以这里完全可以通过<span className="bold">竖直方向的镜像翻转</span>来将滚动条复位。</ArticleDesc>
      <ArticleCode language="css" codeStr={codeString2}></ArticleCode>
      <ArticleDesc>当然，还有另外一种方法可以修复这个问题——css 的 <a href="https://developer.mozilla.org/zh-CN/docs/Web/CSS/direction">direction</a> 属性。这个属性可以设置容器内元素的排列方向，这里我们就可以把滚动容器内的内容和滚动条看作是容器内的两个元素，direction 的默认值是 "ltr" 即从左到右，滚动条的占位可以看作在元素的最后面，翻转后右边的滚动条来到了左边，所以这里只需要将初始的 direction 设置成从右到左 "rtl"，这样翻转后的排列方向就是我们预期的方向了。</ArticleDesc>
      <ArticleDesc>最后还有一个提示，当我在对滚动容器的 direction 设置成 "rtl" 时，需要将容器内子元素的 direction 重置回 "ltr" 的默认值。</ArticleDesc>
      <ArticleCode language="css" codeStr={codeString3}></ArticleCode>
      <ArticleDesc>看看优化后的效果吧：</ArticleDesc>
      <AgentScroller4 fix scale />

      <ArticleDesc>至此，第二种方案也完成了，这种方案的优缺点也逐渐明了：</ArticleDesc>
      <ArticleTitle type="h3" title="优点" />
      <ArticleDesc>仅用样式就可以实现，自动定位到底部，无需计算，相对简单。</ArticleDesc>
      <ArticleTitle type="h3" title="缺点" />
      <ArticleDesc>滚动方向需要额外处理、对话节点需要逆序、影响了默认的布局，后期复杂功能实现成本大。</ArticleDesc>
    </ArticlePartLayout>


    <ArticlePartLayout>
      <ArticleTitle type={'h2'} title={'方案三：column-reverse 翻转内容'} />
      <ArticleDesc>这种方案与第二种方案思路类似，通过另外一种 css 属性 —— flex-direction: column-reverse 实现，实现后的对话式布局与第二种方案大体一致，有一个最主要的差别：column-reverse 是将滚动容器里面的内容进行旋转，而不是将整个滚动容器进行旋转。</ArticleDesc>
      <ArticleTitle type="h3" title="实现步骤" />
      <div className="flex gap-24 fs-12 color-gray-4">
        <div className="flex-1 flex column p-24 gap-16 bg-gray overflow-auto border-box" style={{ height: '220px' }}>
          {Array.from({ length: 3 }).map((_i, idx1) => <div key={idx1} className="bg-gray-3 shrink-0" style={{ height: '120px' }}>对话{idx1 + 1}</div>)}
        </div>
        <div className="flex-1 flex gap-16 p-24 bg-gray overflow-auto border-box" style={{ height: '220px', flexDirection: 'column-reverse' }}>
          {Array.from({ length: 3 }).map((_i, idx1) => <div key={idx1} className="bg-gray-3 shrink-0" style={{ height: '120px' }}>对话{idx1 + 1}</div>)}
        </div>
      </div>
      <div className="flex gap-24 fs-12 color-gray-3">
      ...
      <AgentScroller5 fix />
      <ArticleDesc>其实到这里已经基本实现了想要的效果！实际上只在常规布局的基础上加了一行代码。同样，总结下这种方案的优缺点。</ArticleDesc>
      <ArticleTitle type="h3" title="优点" />
      <ArticleDesc>仅加一行样式就可以实现，不需要额外处理。</ArticleDesc>
      <ArticleTitle type="h3" title="缺点" />
      <ArticleDesc>对话节点需要逆序、影响了默认的布局，后期复杂功能实现成本大。</ArticleDesc>
    </ArticlePartLayout>

    <ArticlePartLayout>
      <ArticleTitle type={'h2'} title={'方案总结'} />
      <ArticleDesc>最后，对上述三种方案先进行第一轮简单总结：（为什么是第一轮呢？因为上述方案只是实现了简单的 Agent 对话式布局，一些更加复杂的功能还没有实现,如果是一个不需要分页加载历史会话、对话内容高度固定的简单 Agent，上述方案完全够用了。）</ArticleDesc>
      <ArticleTable>
        <thead>
          <tr>
            <th>方案</th>
  ...
  <ArticleTitle type={'h1'} title={'二. 上下文分页加载'} />
    <ArticlePartLayout>
      <ArticleDesc>历史会话是 Agent 应用中一个非常常见的功能，现在的主流做法有两种——<span className="bold">单独的历史会话列表</span>（大部分Agent应用的方案）、<span className="bold">上滑分页加载历史会话</span>。</ArticleDesc>
      <ArticleDesc>单独的历史会话列表在实现上没有太大难度，因为历史会话列表已经和生成会话流没有直接的联系了。所以本文主要探索上下文分页加载的场景。与常规的分页加载不同，Agent 应用中由于滚动布局（见上部分内容）的特殊性，分页加载也需要进行特殊处理。上部分内容提到的三种方案对应的分页加载也需要进行不同程度上的处理，才能达到我们预期的效果。</ArticleDesc>
      ...
      <ArticleDesc>可以看到，常规的分页加载在滚动到底部时会触发加载下一页的动作，而纵轴反方向排列的分页加载在滚动到顶部时会触发加载上一页的动作。</ArticleDesc>
      <ArticleTitle type="h4" title="实现步骤" />
      <ArticleDesc>接下来，我们就来看看这两种方案是怎么实现的。</ArticleDesc>
      <ArticleDesc noIndent>step 1: 触发加载：这里我们将【滚动到顶部】转换成【顶部的加载元素全部可见】</ArticleDesc>
      <ArticleCode language="javascript" codeStr={codeString5}></ArticleCode>
      <ArticleDesc noIndent>这一步还有两个可以思考的空间：</ArticleDesc>
      <ArticleDesc>
        <ol>
          <li>🤔是否需要提前加载？通常在常规的列表加载和类似微信的聊天对话页面中，我们可以提前加载数据，弱化用户对加载历史数据的感知，无缝流畅地浏览提前加载好的分页数据。但在Agent对话式布局中，通常会给用户营造更加沉浸的对话环境，等待用户主动触发历史会话的加载。</li>
          <li>触发加载的交互细节——若需要强调用户主动触发加载的操作，可以增加更加细节的交互形式，如下拉回弹加载。</li>
        </ol>
        </ArticleDesc>
      <ArticleDesc noIndent>step 2: 插入数据：这就涉及到我们的布局是用上面哪种方案实现的。</ArticleDesc>
      <ArticleDesc>
        <ul>
          <li><span className="bold">JS 定位底部</span>：因为没有影响常规布局，所以新的对话项正常插入在数据的最后，而历史对话项则需要插入到数据的开头位置，而且，需要逆序插入。</li>
        </ul>
        <ArticleImage url="./images/articles2025/articles2025-3-3-1.png"/>
        <ul>
          <li><span className="bold">rotate or column-reverse</span>：因为影响了常规布局，所以新的对话项需要插入到数据的开头位置，而历史对话项则需要插入到数据的最后位置。</li>
        </ul>
        <ArticleImage url="./images/articles2025/articles2025-3-3-2.png"/>
      </ArticleDesc>
      <ArticleDesc noIndent>接下来还是结合模拟的 Agent 对话来看看效果：</ArticleDesc>
      <ul><li>column-reverse</li></ul>
      <HistoryScroller2 fix/>
      <ul><li>rotate</li></ul>
      <HistoryScroller3/>
      <ArticleDesc>step 4: JS 定位底部方案的特殊处理</ArticleDesc>
      <ArticleDesc>这种方案通过上述简单的逻辑是没办法实现滚动到顶部加载历史会话的，前面说到，这种方案是在常规布局的基础上，通过 JS 在发消息和回答时实时定位到底部，使滚动容器内的所有元素不断向上推。然而在加载历史会话的场景中，可以先忽略新的对话，就相当于一个常规的布局，当我们滚动到顶部触发历史加载时，历史项加入滚动容器中会将原本的内容的内容自动往下顶，这样一来，一方面顶部的加载元素会一直可见，即不断触发历史加载事件，另一方面，加载出来的历史会话位置会自动变动，几乎无法浏览（上述提到使用这种方案的几款产品也都没有上滑加载历史的功能）。</ArticleDesc>
      <ArticleDesc>我们想要的效果是，历史数据加载出来后当前内容不滚动，历史项实际上在之前加载元素的位置。</ArticleDesc>
      <ArticleDesc>先拆解下实现思路——对于一个滚动容器而言，scrollTop是决定其当前位置的关键，要实现视觉上的不滚动，相当于在加载历史数据的瞬间，在页面还没来得及自动往下推之前，计算加上历史数据后页面不滚动对应的新的scrollTop并赋值。</ArticleDesc>
      ...
      <ArticleImage url="./images/articles2025/articles2025-3-3-3.png" imgStyle={{width:'100%'}}/>
      <HistoryScroller1/>
      <ArticleDesc>至此，一个基础的 Agent 布局基本上可以实现了，上述三种方案基本满足大部分 Agent 应用（当然也包括聊天应用）。</ArticleDesc>
    </ArticlePartLayout>
  </ArticleSectionLayout>
  ...
  <ArticleSectionLayout>
    ...
    <ArticlePartLayout>
      ...
      <ArticleDesc>这里需要特别说明的是两种 CSS 方案实现的布局，因为翻转了常规布局，内容的默认行为变成了向上顶，对于展开收起操作而言，这是不符合预期的。</ArticleDesc>
      <ArticleDesc>这里我们以 column-reverse 为例，对比下处理前后展开收起的效果预览：</ArticleDesc>
      <div className="flex gap-24 fs-12 color-gray-4" >
        <FixScroller1/><FixScroller1 fix/>
        </div>
      <ArticleDesc>其实这个场景与刚刚讲到的对于JS定位加载历史的特殊处理类似，都是在内容变化时，将因为布局影响导致的不符合预期的布局变动，手动复位。</ArticleDesc>
      <ArticleDesc>区别在于，前者是在默认向下滚动的布局里通过计算实现向上滚动，后者则反之。</ArticleDesc>
      <ArticleCode codeStr={codeString6} language="javascript"/>
      <ArticleDesc>可以发现，这种对话式布局其实是将最新消息定位底部（默认不支持）的同时，保持用户熟悉的网页操作习惯（浏览器默认行为）。这样一来，不管是用哪种方案实现的对话式布局，都可能存在需要特殊处理的地方。</ArticleDesc>
    </ArticlePartLayout>
    <ArticlePartLayout>
      <ArticleTitle type={'h2'} title={'2. 逐字输出'} />
      {/* <AgentScroller5 autoOutPut customContent={customContent}/> */}
      <ArticleDesc>逐字输出是 Agent 应用中一个非常常见的表现形式，用于展示 Agent 的思考过程。用户对于逐字输出的理解似乎是 Agent 逐字思考、逐字回答，但实际情况下，后台返回的 Agent 回答通常是不定的，可能是一个字，也可能是一句话。如果不做任何处理把回答数据展示给用户，就会出现忽长忽短的“逐字回答”，看上去会有卡顿感。所以，最后展现在用户眼前的逐字回答实际上是前端加工后的效果。</ArticleDesc>
      <ArticleDesc>这里很容易想到实现思路:</ArticleDesc>
      <ArticleDesc><ol><li>将 Agent 实际返回的忽长忽短的数据一次拼接成一个长字符串</li></ol></ArticleDesc>
      <ArticleDesc><ol><li>再将这个长字符串逐字返回</li></ol></ArticleDesc>
      <ArticleCode language="javascript" codeStr={codeString7}/>
      <ArticleDesc><ol><li>配合上逐字返回的动效即可</li></ol></ArticleDesc>
