...
`
export const article2025_1_16 = <ArticleLayout>
    <ArticleSectionLayout>
        <ArticlePreface>在<a href="https://e.qq.com/" style={{color:'#202020'}}>腾讯广告官网（e.qq.com）</a>的页面还原中，通过 SVG 的方式实现了一个支持鼠标交互的轮盘，整个轮盘实现的主要有两个要点：
        <ul>
            <li>扇形的绘制与交互：通过 SVG 的 path 元素（路径）实现。</li>
            <li>扇形交互的过渡效果：内层与外层通过 transform 的 scale 依次扩散。</li>
        </ul>
        趁热打铁，以此为例，对需求中实现轮盘的过程进行封装与扩展，沉淀转盘相关页面的通用重构方案。
        </ArticlePreface>
        <ArticleTitle type={'h2'} title={'一、基础效果预览'}/>
        <ArticlePartLayout>
    ...
    <ArticleSectionLayout>
        <ArticleTitle type={'h2'} title={'二、轮盘扇形分区如何实现？'}/>
        <ArticlePartLayout>
            <ArticleTitle type={'h3'} title={'思路剖析：'}/>
            <ArticleDesc>首先，选择 SVG实现轮盘的原因在于可交互性，使轮盘中的每个扇形都在当前特定的扇形区域内可以用鼠标交互。</ArticleDesc>
            ...
            <ArticleDesc>这样看来，似乎是一个非常非常复杂的组件，且！这样的组件是没有在网页上出现过的。所以层层效果都需要思考应该如何实现，还要保证最终的实现方式是容易维护的。</ArticleDesc>
        </ArticlePartLayout>

        <ArticlePartLayout>
            <ArticleTitle type={'h3'} title={'基础方案（扇形路径）——实现步骤：'}/>
            <ArticleDesc>首先能想到一个很简单的基础方案，将设计稿中的扇形轮盘对应的扇形路径整理成数据直接使用。</ArticleDesc>
            ...
            <ArticleDesc>这种方案显然费时费力且不易维护，一旦任何一个扇形区域的角度变化，数据就需要一起更新。</ArticleDesc>
        </ArticlePartLayout>

        <ArticlePartLayout>
            <ArticleTitle type={'h3'} title={'进阶方案（扇形角度）——实现步骤：'}/>
                <ArticleDesc>更加灵活的做法是参考svg扇形路径的特点，根据角度、半径等数据计算得出对应的扇形路径。</ArticleDesc>
                <ArticleTitle type={'h4'} title={'1. 定义轮盘数据类型'}/>
    ...
    <ArticleSectionLayout>
        <ArticleTitle type={'h2'} title={'三、扩展玩法'}/>
        <ArticleDesc>接下来对这个可以交互交互的扇形轮盘进行一些扩展：</ArticleDesc>
        <ArticlePartLayout>
           <ArticleContentLayout>
             ...
             <SectorWheel 
                ...
                sectorData={[
                    ...
                    id: `sector-${idx}`,
                    color: getRandomRgbaColor(0.3),
                    component: <div className="flex items-center width-100 height-100 pl-24 color-gray-3 fw-600">{idx+1}</div>
                    }))
                ]} 
                startDeg={0} 
                strokeWidth={2}
           ...
           <ArticleContentLayout>
             ...
             <SectorWheel 
                size={400} 
                sectorData={[...[47.5,66,23,23,23,23,23.5,32.5,98.5].map((i,idx) => ({
                    deg: i,
                    id: `sector-${idx+1}`,
             ...
             />
              <ArticleDesc>在组件中新增图片mask层svg，对每个扇形进行图片贴图。</ArticleDesc>
