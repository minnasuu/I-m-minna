    ...
    <ArticleSectionLayout>
      ...
      <ArticlePartLayout>
        ...
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            无压缩的图片格式不对图片数据进行压缩处理，能准确地呈现原图片，例如BMP
            格式。
          </ArticleDesc>
        ...
        <ArticleTable>
          ...
          <tbody>
            {[
              ...
              (
                item: { num1: number; num2: number; example?: string },
                index
              ) => (
                <tr key={index}>
                  <td>{item.num1}</td>
      ...
      <ArticlePartLayout>
        ...
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            GIF格式的图片尺寸较小，支持透明和动画（不支持半透明效果，不支持
            Alpha 通道）。
          </ArticleDesc>
        </ArticleContentLayout>
        <ArticleContentLayout>
        ...
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            由于采用了 8 位压缩，最多只能处理 256
            种颜色，故不宜应用于真彩色(文末的附录有解释)图片。
          </ArticleDesc>
        </ArticleContentLayout>
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            JPEG 格式是最常见的一种图像格式，文件后辍名为“.JPEG”或“.jpg”，JPEG
            可以说是人们最熟悉的图档格式，相信在数字相机普及的现在，几乎每台数字相机、照相手机都可以（甚至只能）输出
            JPEG 格式的图档。
          </ArticleDesc>
          <ArticleDesc>
            JPEG 是一种很典型的使用有损压缩图像格式，也就是说使用者每次进行 JPEG
            的存档动作后，图档的一些内容细节都会遭到永久性的破坏，尤其是使用过高的压缩比例，将使最终解压缩后恢复的图像质量明显降低，如果追求高品质图像，不宜采用过高压缩比例。
          </ArticleDesc>
          <ArticleDesc>
            JPEG
            图片格式的设计目标，是在不影响人类可分辨的图片质量的前提下，尽可能的压缩文件大小。
          </ArticleDesc>
        </ArticleContentLayout>
        <ArticleContentLayout>
          <ArticleTitle type={"h4"} title={"两种JPEG"} />
          <ArticleDesc>
            <span className={"font-bold"}>Baseline JPEG（标准型）：</span>
            按从上到下的扫描方式，把每一行顺序的保存在 JPEG
            文件中。打开这个文件显示它的内容时，数据将按照存储时的顺序从上到下一行一行的被显示出来，直到所有的数据都被读完，就完成了整张图片的显示。
          </ArticleDesc>
          <ArticleDesc>
            <span className={"font-bold"}>Progressive JPEG（渐进式）</span>
            多次扫描，这些扫描顺寻的存储在 JPEG
            文件中。打开文件过程中，会先显示整个图片的模糊轮廓，随着扫描次数的增加，图片变得越来越清晰。
          </ArticleDesc>
        </ArticleContentLayout>
        <ArticleContentLayout>
          <ArticleTitle type={"h4"} title={"😊 优点"} />
          <ArticleDesc>
            <ul>
              <li>
                可以支持 24bit
                真彩色，普遍应用于需要连续色调的图像如色彩丰富的图片、照片等；
              </li>
        ...
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            <ul>
              <li>
                JPEG 不适合用来存储企业
                Logo、线框类的图。因为有损压缩会导致图片模糊，而直接色的选用，又会导致图片文件较GIF更大。
              </li>
        ...
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            便携式网络图形（简称 PNG，英语全称：Portable Network Graphics）。PNG
            能够提供长度比 GIF 小30%的无损压缩图像文件。它同时提供 24 位和 32
            位真彩色图像支持以及其他诸多技术性支持。由于PNG 优秀的特点，PNG
            格式图片可以称为“网页设计专用格式”。PNG 最初的开发目的是为了作为 GIF
            的替代方案的，作为做新开发的影像传输文件格式，PNG
            同样使用了无损压缩格式，事实上 PNG 的开发就是因为 GIF
            所使用的无损压缩格式专利问题而诞生的。
          </ArticleDesc>
          <ArticleTable>
            <thead>
          ...
          <ArticleDesc noIndent>
            区分索引色与直接色：例如，一张 2px*2px
            的超级小图，从左往右从上往下依次的颜色是红，白，白，红。
          </ArticleDesc>
          <ArticleDesc>
            <ul>
              <li>
        ...
        <ArticleContentLayout>
          ...
          <ArticleDesc>
            <span className={"fw-600"}>关键词：</span>
            新格式、有损、无损、兼容性
          </ArticleDesc>
          <ArticleDesc>
            WebP 是谷歌开发的一种新图片格式，WebP
            是同时支持有损和无损压缩的、使用直接色的、点阵图，集多种图片文件格式的优点于一身。
          </ArticleDesc>
          <ArticleDesc>
            <ul>
              <li>
                在无损压缩的情况下，相同质量的 WebP 图片，文件大小要比 PNG
                小26%；
              </li>
              <li>
                在有损压缩的情况下，具有相同图片精度的 WebP 图片，文件大小要比
                JPEG 小 25%~34%；
              </li>
              <li>
                WebP 图片格式支持图片透明度，一个无损压缩的 WebP
                图片，如果要支持透明度只需要 22% 的格外文件大小。
              </li>
            </ul>
          </ArticleDesc>
        </ArticleContentLayout>
